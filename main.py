    ...
    for item in dist_configs:
        try:
            ...
            if r.status_code == 200:
                loc = r.json().get('records', {}).get('locations', [{}])[0].get('location', [{}])[0]
                e = {elem['elementName']: elem['time'][0]['elementValue'][0]['value'] for elem in loc.get('weatherElement', [])}
                results.append(f"📍 {item['name']} {e.get('MinT')}~{e.get('MaxT')}° {e.get('Wx')} (降雨{e.get('PoP12h', '0')}%)")
            else: results.append(f"📍 {item['name']} 氣象局繁忙")
        except: results.append(f"📍 {item['name']} 讀取超時")
    return "\n".join(results)
